...
class Directory
  attr_accessor :dirs, :files, :parent
  ...
  def to_s
    ...
    s += "Files:\n"
    @files.each { |name, size| s += name + ": " + size.to_s + "\n" }
    s += ">"
  end
...
input = File.read('7.input').lines.map(&:strip)
...
min_to_delete = 30_000_000 - (70_000_000 - root.size)

current_smallest_valid = root
to_check = [root]

while (cwd = to_check.pop)
  if cwd.size > min_to_delete and
     cwd.size < current_smallest_valid.size
    current_smallest_valid = cwd
  end

  to_check.concat cwd.dirs.values
end

print current_smallest_valid.size, "\n"
